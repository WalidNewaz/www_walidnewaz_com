...
> warning: Never unpickle untrusted data. It may execute arbitrary code.
...
> note: Best for Python-only data persistence, such as storing trained ML models or temporary cache structures.
...
> Note: Ideal for configuration files (e.g., Docker, Kubernetes, CI/CD pipelines) and human-editable data.

> Tip: Always prefer `safe_load()` over `load()` when parsing YAML.
